...
class Engine:
    ...
    def _register_all(self) -> None:
        """根据当前规则，注册文件监控和定时任务。"""
        # 先清掉旧的：停止所有文件监控 + 删除所有定时任务
        triggers.unwatch_all()
        # 遍历调度器里的任务，逐个删除（简单粗暴但可靠）
        for job in list(triggers._scheduler.get_jobs()):
            # 删除任务（job.id 是任务名）
            triggers._scheduler.remove_job(job.id)

        # 收集需要监控的目录（去重：用集合 set，自动去重）
        ...
        for rule in self.rules:
            ...
            if ttype in (TRIGGER_FILE_ADDED, TRIGGER_FILE_MODIFIED):
                watch_dirs.add(rule["trigger"]["watch_dir"])
            # 定时触发器：注册定时任务（到点执行 handle_schedule，参数是规则 id）
            elif ttype == TRIGGER_SCHEDULE:
                # 用规则的 cron 表达式注册；任务 id 用规则 id（方便以后更新）
                triggers.add_cron_job(
                    lambda rid=rule["id"]: self.handle_schedule(rid),
                    job_id=rule["id"],
                    cron=rule["trigger"]["cron"],
                )

        # 对每个要监控的目录，注册文件监控（回调指向 handle_file_event）
        for d in watch_dirs:
