...
- **支持动态网页**（第 6 阶段增强）：内容靠 JS 加载的现代网站，用系统 Edge 渲染后抓取
- 支持定时自动采集（cron），结果导出 CSV（Excel 直接打开不乱码）
- 内置测试页：`static/test_page.html`（静态）、`static/test_dynamic.html`（动态）
...
| 第 6 阶段 | 爬虫支持动态网页（Playwright + 系统 Edge 渲染） | ✅ 完成（47 个测试通过，exe 已验证） |
